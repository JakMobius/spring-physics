...
public:
    ...
    const std::vector<std::unique_ptr<VertexObject>>& get_vertices() { return m_vertices; }
    const std::vector<std::unique_ptr<SpringObject>>& get_springs() { return m_springs; }
    const std::vector<std::unique_ptr<CreatureTriangleObject>>& get_surfaces() { return m_surfaces; }
    const std::vector<std::unique_ptr<Muscle>>& get_muscles() { return m_muscles; }
    const std::vector<std::unique_ptr<JetObject>>& get_jet_objects() { return m_jet_objects; }
