...
void TerrainBVH::maybe_query_subtree(TerrainBVH::Node* node, Vec3f min, Vec3f max, std::vector<TerrainPolygon*>& result) {
    if (node &&
...
void TerrainBVH::query(TerrainBVH::Node* node, const Vec3f& min, const Vec3f& max, std::vector<TerrainPolygon*>& result) {
    if (node->triangle) {
