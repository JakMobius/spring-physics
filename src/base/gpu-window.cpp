...
bool GpuWindow::is_device_suitable(const VK::PhysicalDevice* device) {
    ...
    if(!device->supports_extensions(required_extensions)) return false;

    if(device->get_queue_family_indices()->find_family(VK_QUEUE_GRAPHICS_BIT) < 0) return false;
    if(device->get_queue_family_indices()->find_surface_present_family(get_vk_surface()) < 0) return false;
    if(!device->has_supported_surface_formats(get_vk_surface())) return false;
    if(!device->has_surface_present_modes(get_vk_surface())) return false;
