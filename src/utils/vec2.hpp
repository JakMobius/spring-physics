...
struct Vec2 {
    ...
    explicit Vec2<T>(content2 newContent)
        : m_content(newContent) {}

    Vec2<T>(T x_, T y_, T z_)
        : m_content{x_, y_, z_} {}
    ...
    inline Vec2<T> operator-() const {
        return Vec2{m_content * -1};
    }

    inline Vec2<T> operator+(const Vec2<T>& second) const {
        return Vec2{m_content + second.m_content};
    }
    ...
    inline Vec2<T> operator*(const T k) const {
        return Vec2<T>{m_content * k};
    }

    inline Vec2<T> operator/(T k) const {
        return Vec2{m_content / k};
    }

    inline Vec2<T>& operator+=(const Vec2<T>& second) {
        m_content += second.m_content;
        return *this;
    }
    ...
    inline Vec2<T>& operator*=(const T k) {
        m_content *= k;
        return *this;
    }
