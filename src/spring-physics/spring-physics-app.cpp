...
SpringPhysicsApp::SpringPhysicsApp() : GeneralApp() {
  ...
  //    m_world->get_surface_mesh().push_back(std::make_unique<SurfaceTriangleObject>(m_world.get(),
  //    Vec3f {-20, 0, -5}, Vec3f {-20, 0, 5}, Vec3f {-100, 5, -5}));
  //    m_world->get_surface_mesh().push_back(std::make_unique<SurfaceTriangleObject>(m_world.get(),
  //    Vec3f {-20, 0, 5}, Vec3f {-100, 5, -5}, Vec3f {-100, 5, 5}));

  // Map with car
  //    {
  //        m_map_object = std::make_unique<MapObject>(m_world.get(),
  //        "resources/maps/map.obj", Matrix4f::scale_matrix(0.01, 0.01, 0.01));
  // On the border (for car & physics testing)
  //        build_model(Matrix4f::translation_matrix(0, 22, 0.35f) *
  //        Matrix4f::rotation_x_matrix(0.1) * Matrix4f::rotation_y_matrix(M_PI
  //        / 2));

  // On the road (for airplane)
  //        build_model(Matrix4f::translation_matrix(0, 25, -3.0f) *
  //        Matrix4f::rotation_x_matrix(0.1) * Matrix4f::rotation_y_matrix(M_PI
  //        / 2));
  //    }
  ...
  {
    m_map_object =
        std::make_unique<MapObject>(m_world.get(), "resources/maps/map2.obj",
                                    Matrix4f::scale_matrix(0.03, 0.03, 0.03) *
                                        Matrix4f::translation_matrix(0, 5, 0));
    // On the border (for car & physics testing)
    build_model({});
  }
  ...
  //                std::make_unique<SurfaceTriangleObject>(m_world.get(),
  //                Vec3f{0, 2.9, -1}, Vec3f{1, 2.9, -1},
  //                                                        Vec3f{0, 2.9, 1}));
  //        m_world->get_surface_mesh().push_back(
  //                std::make_unique<SurfaceTriangleObject>(m_world.get(),
  //                Vec3f{1, 2.9, -1}, Vec3f{0, 2.9, 1},
  //                                                        Vec3f{1, 2.9, 1}));
  //
  ...
  //    load_obj("resources/maps/map2.obj", m_world.get(),
  //    Matrix4f::scale_matrix(0.03, 0.03, 0.03)); build_model({});

  //    m_runway_texture =
  //    m_world->get_renderer()->get_texture_manager()->load_texture("resources/textures/runway.png");
  //    m_runway_texture->make_resident(true);

  //    I'm going to cry.
  //    Freaking OpenGL doesn't support bindless textures on macos.
  //    I'm about to kick the OpenGL from this project and replace it with
  //    Vulkan. Also, it's time to kick out SFML and replace it with GLFW/GLUT.
  //    But first it is worth to publish the OpenGL version, so it won't be lost
  //    completely. Let's do some huge refactoring, baby!

  if (m_creature) {
    m_camera_controller = std::make_unique<FollowCreatureCamera>(
        m_camera.get(), m_creature.get());
  } else {
    m_camera_controller =
        std::make_unique<FreeCameraController>(m_camera.get());
  }
...
void SpringPhysicsApp::create_window(int width, int height) {
  ...
  m_window = std::make_unique<sf::RenderWindow>(
      sf::VideoMode(width, height, 28), "3D Stuff",
...
void SpringPhysicsApp::on_draw() {
  ...
  auto current_controls =
      m_keyboard_input_airplane_controls + m_joystick_airplane_controls;

  m_real_airplane_controls =
      current_controls * 0.1 + m_real_airplane_controls * 0.9;

  if (m_creature) {
    m_creature->set_controls(m_real_airplane_controls);
    m_creature->set_throttle(m_keyboard_throttle + m_joystick_throttle);
  }
...
void SpringPhysicsApp::on_key_press(sf::Keyboard::Key key) {
  ...
  switch (key) {
  case sf::Keyboard::T: {
    m_keyboard_throttle = 1.0f;
    break;
  }
  case sf::Keyboard::Up:
    m_keyboard_input_airplane_controls.y = -1;
    break;
  case sf::Keyboard::Down:
    m_keyboard_input_airplane_controls.y = 1;
    break;
  case sf::Keyboard::Left:
    m_keyboard_input_airplane_controls.x = -1;
    break;
...
void SpringPhysicsApp::on_key_release(sf::Keyboard::Key key) {
  ...
  switch (key) {
  case sf::Keyboard::T: {
    m_keyboard_throttle = 0.0f;
    break;
  }
  case sf::Keyboard::Up:
  case sf::Keyboard::Down:
    m_keyboard_input_airplane_controls.y = 0;
    break;
  case sf::Keyboard::Left:
  case sf::Keyboard::Right:
    m_keyboard_input_airplane_controls.x = 0;
    break;
  case sf::Keyboard::C:
...
void SpringPhysicsApp::build_model(const Matrix4f &transform) {
  ...
  //    m_creature = std::make_unique<NWheelerCreature>(m_world.get(),
  //    transform); m_creature = std::make_unique<TrainCreature>(m_world.get(),
  //    transform);
}
